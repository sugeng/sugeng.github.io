...
function createFile()
{
  ...
  $line   = str_pad('-', 94, '-') . $_lf;
	...
	$hdr .= str_pad("NIM & NAMA", 15, ' ') .
          str_pad(":", 2, ' ') .
          str_pad($header->nim .' '. $header->nama 			, 87, ' ', STR_PAD_RIGHT) .
          $_lf;
	$hdr .= str_pad("JURUSAN", 15,' ') .
          str_pad(":", 2, ' ') .
          str_pad($header->jurusan , 87, ' ', STR_PAD_RIGHT) .
          $_lf;
	$hdr .= str_pad("BATAS STUDI", 15,' ') .
          str_pad(":", 2, ' ') .
          str_pad($header->batas_studi, 87, ' ', STR_PAD_RIGHT) .
          $_lf;
	$hdr .= str_pad("P.A.", 15,' ') .
          str_pad(":", 2, ' ') .
          str_pad($header->dosen_pembimbing	, 87, ' ', STR_PAD_RIGHT) .
          $_lf;
	$hdr .= $line;
  ...
  $hdr .= $line;
	...
	$hdr .= $line;
	$hdr .= str_pad("Dicetak oleh : Hans Solo", 20, ' ') .
          str_pad("Dicetak Tgl : " . date('d-m-Y'), 60,' ', STR_PAD_LEFT).$_lf.$_lf;
	$hdr .= chr(12);
